...
In the Tradecloud One web portal, you can configure your webhooks to use either Basic Authentication, a static Bearer token, or OAuth 2.0 Client Credentials Grant. OAuth is available for order webhooks only and uses the Microsoft Identity Platform (MSAL4J), not a general-purpose OAuth 2.0 client.
