...
After sending an order response to Tradecloud the order lines may get a **new order process status**:

* When`indicators.accepted` is set the order line will have process status`Confirmed`
* When`indicators.rejected` is set the order line will have process status`Rejected`
* When the **responded** `delivery schedule` and `prices` are **equal** to the **requested** `delivery schedule` and `prices` the order line will have process status `Confirmed`
* When the **responded** `delivery schedule` and `prices` are **NOT** equal to the **requested** `delivery schedule` and `prices` the order line will have process status `In Progress`
...
And may get a **new logistics status**:
...
* `deliverySchedule.date`: the responded delivery date of this delivery schedule position. If the delivery date is yet unknown, leave this date empty.  Date has ISO 8601 date `yyyy-MM-dd` format. See also [Standards](../../api/standards.md).
* `deliverySchedule.quantity`: the responded quantity of this delivery schedule position. If the quantity that can be delivered is yet unkown, leave this quantity empty.  Quantity has a decimal `1234.56` format with any number of digits.
...
* `indicators.rejected`: explicitly **reject** the order line, the responded`delivery schedule` and `prices`will be ignored.
* Additional `indicators`:
...
When the `/order-integration/order-response` API method returns HTTP status code 200, the order response was successfully queued to be processed by Tradecloud. Processing takes usually less then a second, after which the order is available in the portal and is forwarded to the buyer ERP integration.
